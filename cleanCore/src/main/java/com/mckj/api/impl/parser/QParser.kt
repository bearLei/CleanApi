...
import com.mckj.api.util.AppUtil
...
class QParser : AbsParser() {
    ...
    override fun parseJunk(
        ...
        for ((key, value) in map) {
            ...
            scanAndroidData(CleanCoreMod.app, value, key) { pkg, bean ->
                details.add(bean)
                totalSize += bean.junkSize
            }
            val icon = AppUtil.getAppIcon(CleanCoreMod.app, key)
        ...
        Log.d(TAG, "整体扫描时间：${System.currentTimeMillis() - startTime}-----$this")
    }
        ...
        try {
            DocumentFile.fromTreeUri(context, uri)?.apply {
                listFiles().forEach {
                    ...
                    Log.d(TAG, "file:${it.name}---url:${it.uri}---是否是文件夹：${it.isDirectory}")
...
//                            } else {
                                Log.d(TAG, "匹配到数据库路径:${junkDbEntity.filePath}--直接计算大小")
                                buildScanResult(
                                    context,
                                    documentId,
                                    it.uri,
...
//                            }
                            ...
                            Log.d(TAG, "匹配到父目录，继续递归：${junkDbEntity.filePath}")
                            listPkg(context, it.uri, pkg, list, consume)
                        }
                    }
                }
            ...
            Log.d(TAG, "listPkg:异常：${e.message}}")
        }
    }
