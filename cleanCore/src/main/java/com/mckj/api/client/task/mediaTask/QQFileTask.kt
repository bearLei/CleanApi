...
class QQFileTask : BaseTask() {
    ...
    override fun scan(consumer: Consumer<AppJunk>): Boolean {
        ...
        try {
            ...
            if (RFileUtils.isAbove30()) {
                documents.addAll(getDocumentsOnQ(CleanCoreMod.app))
            } else {
                getDocuments()?.let {
                    documents.addAll(it)
                }
    ...
    private fun getDocumentsOnQ(context: Context): List<MediaEntity> {
        val list = mutableListOf<MediaEntity>()
        val uri = RFileUtils.getUriTree()
        DocumentFile.fromTreeUri(context, uri)?.uri?.let {
            DocumentFile.fromTreeUri(context, it)?.apply {
                listFiles().forEach { documentFile ->
                    if (documentFile.isDirectory) {
                        ...
                        if (documentId == "${getRootPath()}/com.tencent.mobileqq") {
                            listQQ(context,documentFile.uri) { entity ->
                                list.add(entity)
                            }
                        }
                    }
    ...
    private fun listQQ(context: Context,uri: Uri, consume: Consumer<MediaEntity>) {
            ...
            listFiles().forEach {
                if (it.isDirectory) {
                    ...
                    if (documentId == "${getRootPath()}/com.tencent.mobileqq/Tencent/QQfile_recv") {
                        DocumentFile.fromTreeUri(context, it.uri)?.apply {
